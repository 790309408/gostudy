...
import "go-study/rulego/api/types/metrics"

// RuleEngineOption defines a function type for configuring a RuleEngine.
// RuleEngineOption 定义了用于配置 RuleEngine 的函数类型。
// type RuleEngineOption：声明了一个新的类型，名为 RuleEngineOption
// func(RuleEngine) error：表示这个类型本质是一个函数，它接收一个 RuleEngine 类型的参数，返回一个 error
