...
var (
	ErrSshConfigEmpty   = errors.New("ssh config can not empty")
	ErrSshClientNotInit = errors.New("ssh client not initialized")
	ErrSshCmdEmpty      = errors.New("cmd can not empty")
)
...
func (x *SshNode) OnMsg(ctx types.RuleContext, msg types.RuleMsg) {
	var err error
	if x.client == nil {
		ctx.TellFailure(msg, ErrSshClientNotInit)
		return
	}
	// 获取shell 命令
	cmd := x.Config.Cmd
	if cmd == "" {
		ctx.TellFailure(msg, ErrSshCmdEmpty)
		return
	}
