...
import (
	...
	"go-study/rulego/components/js"
	"go-study/rulego/utils/json"
	"go-study/rulego/utils/maps"
	string2 "go-study/rulego/utils/str"
)

// ErrJsTransformReturnFormat 如果脚本返回值这个格式错误：return {'msg':msg,'metadata':metadata,'msgType':msgType}
var ErrJsTransformReturnFormat = errors.New("return the value is not a map")
...
func (x *JsTransformNode) OnMsg(ctx types.RuleContext, msg types.RuleMsg) {
	...
	if err != nil {
		...
		if ok {
			...
			ctx.TellFailure(msg, ErrJsTransformReturnFormat)
		}
	}
