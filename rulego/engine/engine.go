...
import (
	...
	"go-study/rulego/api/types/metrics"

	"go-study/rulego/builtin/aspect"
...
// newRuleEngine 使用给定的 ID 和定义创建一个新的 RuleEngine 实例。
...
// 它应用任何提供的 RuleContextOptions 来定制执行上下文
...
func NewConfig(opts ...types.Option) types.Config {
	// opts... 在切片后加上 ...，表示将切片展开为多个独立参数传递给可变参数函数
	c := types.NewConfig(opts...)
	if c.Parser == nil {
		c.Parser = &JsonParser{} //创建一个默认的解析器
	}
	if c.ComponentsRegistry == nil {
		c.ComponentsRegistry = Registry // 注册所有内置组件  默认为 `rulego.Registry`
	}
	// register all udfs
	// 注册所有 udf
...
// WithConfig 是一个设置 RuleEngine 的配置的选项。
